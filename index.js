...
function Storage (chunkLength, opts) {
  ...
  if (opts.files) {
    ...
    self.files = opts.files.slice(0).map(function (file, i, files) {
      ...
      if (self.path) file.path = path.resolve(path.join(self.path, self.name, file.path))
      return file
    })
    ...
    self.files = [{
      offset: 0,
      path: path.resolve(self.path || path.join(TMP, self.name)),
      length: len
    }]
    self.length = len
  }
...
Storage.prototype.destroy = function (cb) {
  ...
  self.close(function () {
    if (self.path) {
      fs.rm(path.resolve(path.join(self.path, self.name)), { recursive: true, maxRetries: 10 }, function (err) {
